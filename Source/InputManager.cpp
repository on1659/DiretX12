...
void CInputManager::Load()
{
	...
	CInputManager::AddKey(YK_C, VK_C, GPAD_C);
	...
	CInputManager::AddKey(YK_G, VK_G, GPAD_G);

	CInputManager::AddKey(YK_0, VK_0);
	CInputManager::AddKey(YK_1, VK_1, GPAD_1);
	CInputManager::AddKey(YK_2, VK_2, GPAD_2);
	CInputManager::AddKey(YK_3, VK_3, GPAD_3);
	CInputManager::AddKey(YK_4, VK_4, GPAD_4);
	...
	CInputManager::AddKey(YK_TAB, VK_TAB, GPAD_BACK);
	...
	CInputManager::AddKey(YK_CTRL, VK_CONTROL);
}
...
Add PadButton
*/
void CInputManager::AddPadButton(YT_KEY pad, BYTE input)
{
	tagKeyState keystate;
	keystate.BKey = input;
...
bool CInputManager::OnlyKeyPadDown(YT_KEY input)
{
	...
	*/
	...
	if (input == YT_KEY::YK_SPACE)	 press = (m_xGamepadState.Gamepad.bLeftTrigger != NULL);
	else if (input == YT_KEY::YK_LSHIFT) press = (m_xGamepadState.Gamepad.bRightTrigger != NULL);
...
void CInputManager::InputDataSending()
{
	while (!m_bInputQueue.empty())
	{
		...
		*/
		...
		//스킬 키 및 아이템 사용 키
		if (
			check.key == YT_KEY::YK_SPACE //뒤통수 때리기
			|| check.key == YT_KEY::YK_RBUTTON //은신
			|| check.key == YT_KEY::YK_G //액티브 스킬
			|| check.key == YK_1 || check.key == YK_2 || check.key == YK_3 || check.key == YK_4
			)
			//|| (IsGamepadConnected() && NULL != GetGamepad().bLeftTrigger))
		{
			P_CS_INGAME_OTHER_INPUT_EVENT cs_ingame_skill_event;
			cs_ingame_skill_event.size = sizeof(P_CS_INGAME_OTHER_INPUT_EVENT);
			cs_ingame_skill_event.type = CS_INGAME_OTHER_INPUT_EVENT;
			cs_ingame_skill_event.keytype = check.key;
			cs_ingame_skill_event.is_keydown = check.bDown;
			TCP->SendPacket((unsigned char*)&cs_ingame_skill_event);
		}

		//이동 키
		else
		{
			P_CS_INGAME_MOVE_INPUT_EVENT cs_ingame_move_event;
			cs_ingame_move_event.size = sizeof(P_CS_INGAME_MOVE_INPUT_EVENT);
			cs_ingame_move_event.type = CS_INGAME_MOVE_INPUT_EVENT;
			cs_ingame_move_event.keytype = check.key;
			cs_ingame_move_event.is_keydown = check.bDown;
			TCP->SendPacket((unsigned char*)&cs_ingame_move_event);
		}
...
void CInputManager::StartLeftVibrate(int vibrate)
{
	...
	Vibration.wRightMotorSpeed = 0.0f;
	XInputSetState(0, &Vibration);
	m_bVibrate = true;
}
...
void CInputManager::StartRightVibrate(int vibrate)
{
	...
	m_bVibrate = true;
	XINPUT_VIBRATION Vibration;
	Vibration.wLeftMotorSpeed = 0.0f;
