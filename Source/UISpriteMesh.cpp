...
CUISpriteMesh::CUISpriteMesh(ID3D11Device *pd3dDevice, float fWidth, float fHeight) : CMeshTexturedIlluminated(pd3dDevice)
{
	m_nVertices = 6;
	...
	m_f3BeginPosition = m_fSize;

	m_pd3dxvPositions = new XMFLOAT3[m_nVertices];
	int i = 0;

	m_pd3dxvPositions[i++] = XMFLOAT3(-fx, +fy, -fz); //ÁÂ»ó 0
	m_pd3dxvPositions[i++] = XMFLOAT3(+fx, +fy, -fz); //¿ì»ó 1
	m_pd3dxvPositions[i++] = XMFLOAT3(+fx, -fy, -fz); //¿ìÇÏ 2

	m_pd3dxvPositions[i++] = XMFLOAT3(-fx, +fy, -fz); //ÁÂ»ó 3
	m_pd3dxvPositions[i++] = XMFLOAT3(+fx, -fy, -fz); //¿ìÇÏ 4
	m_pd3dxvPositions[i++] = XMFLOAT3(-fx, -fy, -fz); //ÁÂÇÏ 5

	m_dot[0] = XMFLOAT3(-fx, +fy, -fz);
	m_dot[1] = XMFLOAT3(+fx, +fy, -fz);
	m_dot[2] = XMFLOAT3(+fx, -fy, -fz);
...
CUISpriteMesh::~CUISpriteMesh()
{
}

void CUISpriteMesh::GetDot(XMFLOAT3 &a, XMFLOAT3 &b, XMFLOAT3 &c)
{
	a = m_dot[0];	b = m_dot[1];	c = m_dot[2];
}
