...
float4 OutLineColor(float2 tex)
{
    float fWidth = gOutLineWidth;
    if (fWidth <= 0.0f)
        return false;
    ...
    float4 color = 0.0f;
    color = (gtxtDiffuse.Sample(gssSamplerState, float2(tex.x * minus.x, tex.y * minus.y)));
    if (color.a >= 15.0f) return float4(color.xyz * gOutLineColor, 1.0f);
    color = (gtxtDiffuse.Sample(gssSamplerState, float2(tex.x, tex.y * minus.y)));
      if (color.a >= 15.0f) return float4(color.xyz * gOutLineColor, 1.0f);
  
    color = (gtxtDiffuse.Sample(gssSamplerState, float2(tex.x * plus.x, tex.y * minus.y)));
       if (color.a >= 15.0f) return float4(color.xyz * gOutLineColor, 1.0f);
   
    color = (gtxtDiffuse.Sample(gssSamplerState, float2(tex.x * minus.x, tex.y)));
      if (color.a >= 15.0f) return float4(color.xyz * gOutLineColor, 1.0f);

   color = (gtxtDiffuse.Sample(gssSamplerState, float2(tex.x * plus.x, tex.y)));
      if (color.a >= 15.0f) return float4(color.xyz * gOutLineColor, 1.0f);
   
    color = (gtxtDiffuse.Sample(gssSamplerState, float2(tex.x * minus.x, tex.y * plus.y)));
      if (color.a >= 15.0f) return float4(color.xyz * gOutLineColor, 1.0f);

    color = (gtxtDiffuse.Sample(gssSamplerState, float2(tex.x, tex.y * plus.y)));
       if (color.a >= 15.0f) return float4(color.xyz * gOutLineColor, 1.0f);
 
      color = (gtxtDiffuse.Sample(gssSamplerState, float2(tex.x * plus.x, tex.y * plus.y)));
       if (color.a >= 15.0f) return float4(color.xyz * gOutLineColor, 1.0f);
  
    color = (gtxtDiffuse.Sample(gssSamplerState, float2(tex.x, tex.y)));
     return color;
}
...
/*
fxc /E PSTextureToScreen /T ps_5_0 /Od /Zi /Fo CompiledGs.fxo ScreenShaderFreeCamera.hlsl
*/
